...
void Dash::DrawBackground(Adafruit_RA8875 tft, int16_t color) {
    ...
    this->command_buffer.addCommand(DrawCommand::fillScreen(color));
    ...
    // draw outlines
    // MIDDLE DRIVE RECT
    this->command_buffer.addCommands((std::vector<DrawCommand>){
        DrawCommand::drawRect(SCREEN_WIDTH / 4, SCREEN_HEIGHT / 3, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 3, RA8875_WHITE),
        // COOLANT, MAX CELL, INTERVERT TEMPS RECT
        DrawCommand::drawRect(SCREEN_WIDTH / 4, SCREEN_HEIGHT * 2 / 3, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 3, RA8875_WHITE),
        // MOTOR TEMP CIRC TOP LEFT
        DrawCommand::drawCircle(SCREEN_WIDTH / 8, SCREEN_HEIGHT / 4, SCREEN_WIDTH / 8, RA8875_WHITE),
        // ACCUM TEMP CIRC BOTTOM LEFT
        DrawCommand::drawCircle(SCREEN_WIDTH / 8, SCREEN_HEIGHT * 3 / 4, SCREEN_WIDTH / 8, RA8875_WHITE),
        // MIN VOLTAGE CIRC TOP RIGHT
        DrawCommand::drawCircle(SCREEN_WIDTH * 7 / 8, SCREEN_HEIGHT / 4, SCREEN_WIDTH / 8, RA8875_WHITE),
        // BATTERY VOLTAGE CIRC BOTTOM RIGHT
        DrawCommand::drawCircle(SCREEN_WIDTH * 7 / 8, SCREEN_HEIGHT * 3 / 4, SCREEN_WIDTH / 8, RA8875_WHITE),
...
int inverter_temp = static_cast<int>(inverter_temp_status_igbt_temp);
int motor_temp = static_cast<int>(inverter_temp_status_motor_temp); 
...
float inverter_current_drawn=static_cast<float>(inverter_current_draw_ah_drawn); //gotta change in dash.h too
#else
    ...
    this->command_buffer.drawNext(tft);
}
...
void Dash::DrawCoolantTemp(Adafruit_RA8875 tft, float coolant_temp, int startX, int startY) {
    this->command_buffer.addCommand(DrawCommand::drawText(startX+2,startY,5,RA8875_WHITE, RA8875_BLACK, "CT", static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
...
void Dash::DrawMaxCellTemp(Adafruit_RA8875 tft, float max_cell_temp, int startX, int startY) {
    this->command_buffer.addCommand(DrawCommand::drawText(startX+2,startY,5,RA8875_WHITE, RA8875_BLACK,"MCT", static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT))); 
...
void Dash::DrawInverterTemp(Adafruit_RA8875 tft, float inverter_temp, int startX, int startY) {
    this->command_buffer.addCommand(DrawCommand::drawText(startX+2,startY,5,RA8875_WHITE, RA8875_BLACK,"IT", static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
...
void Dash::DrawWheelSpeed(Adafruit_RA8875 tft, float wheel_speed, int startX, int startY) {
    ...
    this->command_buffer.addCommand(DrawCommand::fillRect(startX - 150, startY, 300, 110, RA8875_WHITE));
    ...
    while (rounded_wheel_speed > 0) {
        ...
        this->command_buffer.addCommand(DrawCommand::drawText(startX, startY, 13, RA8875_BLACK,RA8875_WHITE, digit_str, static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
...
void Dash::DrawDriveState(Adafruit_RA8875 tft, int startX, int startY, int curr_drive_state, int squareSize) {
    ...
    switch (curr_drive_state) {
        case 0:
            char digit_str_drive[6];  
            strcpy(digit_str_drive, "DRIVE");
            this->command_buffer.addCommand(DrawCommand::drawText(SCREEN_WIDTH * 0.4, SCREEN_HEIGHT * 0.58, 5, RA8875_BLACK,color, digit_str_drive, static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
            break;
        case 1:
            char digit_str_on[3];  
            strcpy(digit_str_on, "ON");
            this->command_buffer.addCommand(DrawCommand::drawText(SCREEN_WIDTH * 0.47, SCREEN_HEIGHT * 0.58, 5, RA8875_BLACK, color, digit_str_on, static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
            break;
        case 2:
            char digit_str_off[4];  
            strcpy(digit_str_off, "OFF");
            this->command_buffer.addCommand(DrawCommand::drawText(SCREEN_WIDTH * 0.45, SCREEN_HEIGHT * 0.58, 5, RA8875_BLACK, color, digit_str_off, static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
            break;
    }
...
void Dash::DrawAccumTempState(Adafruit_RA8875 tft, int startX, int startY, int curr_accum_state, int squareSize) {
    ...
    this->command_buffer.addCommand(DrawCommand::fillCircle(SCREEN_WIDTH / 8, SCREEN_HEIGHT * 3 / 4, SCREEN_WIDTH / 8, color));
    char digit_str_AT[6];  
    strcpy(digit_str_AT, "AT");
    this->command_buffer.addCommand(DrawCommand::drawText(startX * 0.8, startY - SCREEN_WIDTH / 9, 5, RA8875_BLACK, color, digit_str_AT, static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
...
void Dash::DrawMinVolt(Adafruit_RA8875 tft, float min_voltage, int startX, int startY) {
    ...
    while (rounded_min_voltage > 0) {
        ...
        char digit_char = '0' + digit;
        char digit_str[2]={digit_char,'\0'};
        this->command_buffer.addCommand(DrawCommand::drawText(startX + 4, startY * 0.8, 11, RA8875_BLACK, RA8875_WHITE, digit_str, static_cast<TextDirection>(Dash::Direction::LEFT_TO_RIGHT)));
        startX -= char_width + digit_spacing;
